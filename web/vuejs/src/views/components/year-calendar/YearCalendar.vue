...
export default {
  ...
  methods: {
    changeYear (idx) {
      ...
      var selectedYear = idx + this.activeYear - 3;

      if (this.minYear > 0 && (selectedYear < this.minYear) ) {
        return;
      }

      if (this.maxYear > 0 && (selectedYear > this.maxYear)) {
        return;
      }

      this.activeYear = idx + this.activeYear - 3;
      this.$emit('changeYear', {
        prevYear:this.activeYear
      })
    },
    toggleDate (dateObj) {
      ...
      if ((day === 6) || (day === 0)) {
        if (!confirm(this.$store.state.t('Do yo want to change weekend status?'))) {
          return
        }
      }
