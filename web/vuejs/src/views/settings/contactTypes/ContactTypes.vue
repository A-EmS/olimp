<template>
  <div>
    <page-title :button-action-hide="getACL().create !== true" :createProcessName="createProcessName" :heading="$store.state.t('Contact Types')" :subheading="$store.state.t('Contact Types actions')" icon='pe-7s-id icon-gradient bg-happy-itmeo' :starShow=false></page-title>
    ...
    <b-card v-if="getACL().list === true" :title="$store.state.t('Contact Types')" class="main-card mb-4">
      <b-row class="mb-3">
        ...
        <template slot="actions" slot-scope="row">
          <table>
            <tr>
              ...
              <td v-if="getACL().delete === true"><i class='lnr-trash' size="sm" style="cursor: pointer; font-size: large; color: red" @click.stop="" @click="confirmDeleteRow(parseInt(row.item.id), row.item.contact_type)"> </i></td>
            </tr>
          </table>
        </template>
      </b-table>
