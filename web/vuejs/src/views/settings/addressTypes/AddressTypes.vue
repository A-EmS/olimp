<template>
  <div>
    <page-title :button-action-hide="getACL().create !== true" :createProcessName="createProcessName" :heading="$store.state.t('Address Types')" :subheading="$store.state.t('Address Types actions')" icon='pe-7s-id icon-gradient bg-happy-itmeo' :starShow=false></page-title>
    ...
    <b-card v-if="getACL().list === true" :title="$store.state.t('Address Types')" class="main-card mb-4">
      <b-row class="mb-3">
        ...
        <template slot="actions" slot-scope="row">
          <table>
            <tr>
              ...
              <td v-if="getACL().delete === true"><i class='lnr-trash' size="sm" style="cursor: pointer; font-size: large; color: red" @click.stop="" @click="confirmDeleteRow(parseInt(row.item.id), row.item.address_type)"> </i></td>
            </tr>
          </table>
        </template>
      </b-table>
...
<script>
  ...
  export default {
    ...
    data: () => ({
      ...
      updateItemListEventName: 'updateList:addressTypes',
      createProcessName: 'create:addressType',
      updateProcessName: 'update:addressType',
      confirmatorInputProcessName: 'confirm:addressType',
      confirmatorOutputProcessName: 'confirmed:addressType',
    ...
    methods: {
      getAddressTypes: function () {
        axios.get(window.apiDomainUrl+'/address-types/get-all', qs.stringify({}))
                .then( (response) => {
      ...
      confirmDeleteRow: function(id, name){
        this.$eventHub.$emit(this.confirmatorInputProcessName, {
          ...
          confirmString: this.$store.state.t('Confirm delete') +  ' ' + this.$store.state.t('Address Types') +'..'+name,
          idToConfirm: id
        });
      },
      ...
      deleteRow: function(id){
        ...
        axios.post(window.apiDomainUrl+'/address-types/delete', qs.stringify({id:id}))
                .then( (response) => {
                  if(response.data !== false){
