<template>
  <div>
    <page-title :button-action-hide="getACL().create !== true" :createProcessName="createProcessName" :heading="$store.state.t('Document Types')" :subheading="$store.state.t('Document Types actions')" icon='pe-7s-global icon-gradient bg-happy-itmeo' :starShow=false></page-title>
    ...
    <b-card v-if="getACL().list === true" :title="$store.state.t('Document Types')" class="main-card mb-4">
      <b-row class="mb-3">
  ...
  export default {
    ...
    methods: {
      ...
      getDocumentTypes: function () {
        this.documentTypesManager.getAll()
          .then( (response) => {
            if(response.data !== false){
      ...
      deleteRow: function(id){
        ...
        this.documentTypesManager.delete({id:id})
                .then( (response) => {
                  if(response.data !== false){
