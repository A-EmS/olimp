<template>
  <div>
    <page-title :button-action-hide="getACL().create !== true" :createProcessName="createProcessName" :heading="$store.state.t('Patterns')" :subheading="$store.state.t('Patterns actions')" icon='pe-7s-id icon-gradient bg-happy-itmeo' :starShow=false></page-title>
    ...
    <b-card v-if="this.getACL().list === true" :title="$store.state.t('Patterns')" class="main-card mb-4">
      <b-row class="mb-3">
...
<script>
  ...
  export default {
    ...
    data: () => ({
      ...
      updateItemListEventName: 'updateList:patterns',
      createProcessName: 'create:patterns',
      updateProcessName: 'update:patterns',
      configureProcessName: 'configure:patterns',
      confirmatorInputProcessName: 'confirm:patterns',
      confirmatorOutputProcessName: 'confirmed:patterns',
    ...
    methods: {
      ...
      confirmDeleteRow: function(id, name){
        this.$eventHub.$emit(this.confirmatorInputProcessName, {
          ...
          confirmString: this.$store.state.t('Confirm delete') +  ' ' + this.$store.state.t('Pattern') +'..'+name,
          idToConfirm: id
        });
      },
      ...
      deleteRow: function(id){
        ...
        this.patternsManager.delete({id:id})
                .then( (response) => {
                  if(response.data !== false){
      ...
      setDefaultInterfaceData: function () {
        ...
        this.fields = [
          ...
          { key: 'name', label: this.$store.state.t('Name')},
