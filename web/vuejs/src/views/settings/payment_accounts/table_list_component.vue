    ...
    <div>
        <b-card :title="getCardTitle()" class="main-card mb-4">
            ...
            <button style="margin-bottom: 10px" v-if="getACL().create === true && notOriginalPage && showAdditionalCreatingButton" v-on:click="createNew()" type="button" class="btn-shadow d-inline-flex align-items-center btn btn-success">
                {{$store.state.t('Add Payment Account')}}
            </button>
                ...
                <template slot="actions" slot-scope="row">
                    <table>
                        <tr>
                            ...
                            <td v-if="getACL().delete === true"><i class='lnr-trash' size="sm" style="cursor: pointer; font-size: large; color: red" @click.stop="" @click="confirmDeleteRow(parseInt(row.item.id), row.item.contractor_name+' '+row.item.bank_name)"> </i></td>
                        </tr>
                    </table>
                </template>
            </b-table>
  ...
  export default {
    ...
    data () {
      return {
          ...
          updateItemListEventName: 'updateList:paymentAccounts',
          createProcessName: 'create:paymentAccount',
          updateProcessName: 'update:paymentAccount',
          confirmatorInputProcessName: 'confirm:paymentAccount',
          confirmatorOutputProcessName: 'confirmed:paymentAccount',
    ...
    methods: {
        ...
        getCardTitle: function () {
            if (this.showCardTitle){
                return this.$store.state.t('Payment Account');
            }
        ...
        getDataForList: function () {
            ...
            this.paymentAccountsManager.getAll(conditionalString)
                .then( (response) => {
        ...
        confirmDeleteRow: function(id, name){
            this.$eventHub.$emit(this.confirmatorInputProcessName, {
                ...
                confirmString: this.$store.state.t('Confirm delete') +  ' ' + this.$store.state.t('Payment Account') +'..'+name,
                idToConfirm: id
            });
        },
        ...
        deleteRow: function(id){
            ...
            this.paymentAccountsManager.deleteRow(id)
                .then( (response) => {
                    if(response.data !== false){
