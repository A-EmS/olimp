<template>
  <div>
    <page-title :button-action-hide="getACL().create !== true" :createProcessName="createProcessName" :heading="$store.state.t('Individuals')" :subheading="$store.state.t('Individuals actions')" icon='pe-7s-user icon-gradient bg-happy-itmeo' :starShow=false></page-title>

    <form_component v-if="getACL().update === true" :showListEventName="showListEventName" :createProcessNameTrigger="createProcessName" :updateProcessNameTrigger="updateProcessName" :updateItemListNameTrigger="updateItemListEventName" ></form_component>

    <b-card v-if="getACL().list === true" v-show="showList" :title="$store.state.t('Individuals')" class="main-card mb-4">
      <b-row class="mb-3">
