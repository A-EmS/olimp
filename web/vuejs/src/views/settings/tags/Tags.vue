<template>
  <div>
    <page-title :button-action-hide="getACL().create !== true" :createProcessName="createProcessName" :heading="$store.state.t('Tags')" :subheading="$store.state.t('Tags actions')" icon='pe-7s-id icon-gradient bg-happy-itmeo' :starShow=false></page-title>
    ...
    <b-card v-if="this.getACL().list === true" :title="$store.state.t('Tags')" class="main-card mb-4">
      <b-row class="mb-3">
...
<script>
  ...
  export default {
    ...
    data: () => ({
      ...
      updateItemListEventName: 'updateList:tags',
      createProcessName: 'create:tags',
      updateProcessName: 'update:tags',
      configureProcessName: 'configure:tags',
      confirmatorInputProcessName: 'confirm:tags',
      confirmatorOutputProcessName: 'confirmed:tags',
    ...
    methods: {
      ...
      confirmDeleteRow: function(id, name){
        this.$eventHub.$emit(this.confirmatorInputProcessName, {
          ...
          confirmString: this.$store.state.t('Confirm delete') +  ' ' + this.$store.state.t('Tag') +'..'+name,
          idToConfirm: id
        });
      },
