<template>
  <div>
    <page-title :button-action-hide="getACL().create !== true" :createProcessName="createProcessName" :heading="$store.state.t('Own Companies')" :subheading="$store.state.t('Own Companies actions')" icon='pe-7s-global icon-gradient bg-happy-itmeo' :starShow=false></page-title>
    ...
    <b-card v-if="getACL().list === true" :title="$store.state.t('Own Companies')" class="main-card mb-4">
      <b-row class="mb-3">
        ...
        <template slot="actions" slot-scope="row">
          <table>
            <tr>
              ...
              <td v-if="getACL().delete === true"><i class='lnr-trash' size="sm" style="cursor: pointer; font-size: large; color: red" @click.stop="" @click="confirmDeleteRow(parseInt(row.item.id), row.item.company)"> </i></td>
            </tr>
          </table>
        </template>
      </b-table>
...
<script>
  ...
  export default {
    ...
    data: () => ({
      ...
      updateItemListEventName: 'updateList:ownCompanies',
      createProcessName: 'create:ownCompanies',
      updateProcessName: 'update:ownCompanies',
      confirmatorInputProcessName: 'confirm:ownCompanies',
      confirmatorOutputProcessName: 'confirmed:ownCompanies',
    ...
    methods: {
      ...
      confirmDeleteRow: function(id, company){
        this.$eventHub.$emit(this.confirmatorInputProcessName, {
          titleString: this.$store.state.t('Deleting') + '...',
          confirmString: this.$store.state.t('Confirm delete') +  ' ' + this.$store.state.t('Own Companies') +'..'+company,
          idToConfirm: id
        });
      },
      ...
      deleteRow: function(id){
        ...
        this.ownCompaniesManager.deleteRow(id)
                .then( (response) => {
                  if(response.data !== false){
      ...
      setDefaultInterfaceData: function () {
        ...
        this.fields = [
          ...
          { key: 'company', label: this.$store.state.t('Company'), sortable: true},
