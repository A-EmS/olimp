<template>
  <div>
    <page-title :button-action-hide="getACL().create !== true" :createProcessName="createProcessName" :heading="$store.state.t('Currency Exchange Rates')" :subheading="$store.state.t('Currency Exchange Rates actions')" icon='pe-7s-global icon-gradient bg-happy-itmeo' :starShow=false></page-title>
    ...
    <b-card v-if="getACL().list === true" :title="$store.state.t('Currency Exchange Rates')" class="main-card mb-4">
      <b-row class="mb-3">
...
export default {
  ...
  data: () => ({
    ...
    updateItemListEventName: 'updateList:currencyExchangeRates',
    createProcessName: 'create:currencyExchangeRates',
    updateProcessName: 'update:currencyExchangeRates',
    confirmatorInputProcessName: 'confirm:deleteCurrencyExchangeRates',
    confirmatorOutputProcessName: 'confirmed:deleteCurrencyExchangeRates',
  ...
  methods: {
    getCurrenciesRates: function () {
      this.currencyExchangeRatesManager.getAll()
          .then( (response) => {
    ...
    confirmDeleteRow: function(id, name){
      this.$eventHub.$emit(this.confirmatorInputProcessName, {
        ...
        confirmString: this.$store.state.t('Confirm delete') +  ' ' + this.$store.state.t('Currency Exchange Rates') +'..'+name,
        idToConfirm: id
      });
    },
    ...
    deleteRow: function(id){
      ...
      this.currencyExchangeRatesManager.delete({id:id})
          .then( (response) => {
            if(response.data !== false){
    ...
    setDefaultInterfaceData: function () {
      ...
      this.fields = [
        ...
        { key: 'currency_base', label: this.$store.state.t('Currency Base'), sortable: true},
        { key: 'rate_base', label: this.$store.state.t('Rate'), sortable: true},
        { key: 'currency_ref', label: this.$store.state.t('Currency Ref'), sortable: true},
        { key: 'rate_ref', label: this.$store.state.t('Rate'), sortable: true},
        { key: 'date_from', label: this.$store.state.t('Date From'), sortable: true},
        { key: 'date_to', label: this.$store.state.t('Date To'), sortable: true},
