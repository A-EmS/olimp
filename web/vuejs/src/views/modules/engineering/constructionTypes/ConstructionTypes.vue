<template>
  <div>
    <page-title :button-action-hide="getACL().create !== true" :createProcessName="createProcessName" :heading="$store.state.t('Construction Types')" :subheading="$store.state.t('Construction Types actions')" icon='pe-7s-global icon-gradient bg-happy-itmeo' :starShow=false></page-title>
    ...
    <b-card v-if="getACL().list === true" :title="$store.state.t('Construction Types')" class="main-card mb-4">
      <b-row class="mb-3">
...
<script>
  ...
  export default {
    ...
    data: () => ({
      ...
      updateItemListEventName: 'updateList:constructionTypes',
    ...
    methods: {
      ...
      confirmDeleteRow: function(id, name){
        this.$eventHub.$emit(this.confirmatorInputProcessName, {
          ...
          confirmString: this.$store.state.t('Confirm delete') +  ' ' + this.$store.state.t('Construction Type') +'..'+name,
          idToConfirm: id
        });
      },
      ...
      deleteRow: function(id){
        ...
        this.constructionTypesManager.delete({id:id})
                .then( (response) => {
                  if(response.data !== false){
      ...
      setDefaultInterfaceData: function () {
        ...
        this.fields = [
          ...
          { key: 'name', label: this.$store.state.t('Construction Type'), sortable: true},
