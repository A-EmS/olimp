<template>
  <div>
    <page-title :button-action-hide="getACL().create !== true" :createProcessName="createProcessName" :heading="$store.state.t('Project Statuses')" :subheading="$store.state.t('Project Statuses actions')" icon='lnr-list icon-gradient bg-happy-itmeo' :starShow=false></page-title>
    ...
    <b-card v-if="getACL().list === true" :title="$store.state.t('Project Statuses')" class="main-card mb-4">
      <b-row class="mb-3">
        ...
        <template slot="actions" slot-scope="row">
          <table>
            <tr>
              ...
              <td v-if="getACL().delete === true"><i class='lnr-trash' size="sm" style="cursor: pointer; font-size: large; color: red" @click.stop="" @click="confirmDeleteRow(parseInt(row.item.id), row.item.status)"> </i></td>
            </tr>
          </table>
        </template>
      </b-table>
...
<script>
  ...
  export default {
    ...
    data: () => ({
      ...
      updateItemListEventName: 'updateList:projectStatuses',
    ...
    methods: {
      ...
      confirmDeleteRow: function(id, name){
        this.$eventHub.$emit(this.confirmatorInputProcessName, {
          ...
          confirmString: this.$store.state.t('Confirm delete') +  ' ' + this.$store.state.t('Project Statuses') +'..'+name,
          idToConfirm: id
        });
      },
      ...
      deleteRow: function(id){
        ...
        this.projectStatusesManager.delete({id:id})
                .then( (response) => {
                  if(response.data !== false){
