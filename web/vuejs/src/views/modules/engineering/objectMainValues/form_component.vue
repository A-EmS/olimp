      ...
      <demo-card :heading="header" subheading="">
        ...
        >
          ...
          <v-text-field
              ...
              :counter="250"
              :label="$store.state.t('Object Main Value')"
              required
              @input="$v.name.$touch()"
              @blur="$v.name.$touch()"
          ></v-text-field>
  ...
  export default {
    ...
    created() {
      ...
      this.$eventHub.$on(this.updateProcessNameTrigger, (data) => {
        this.initFormComponent();
        this.objectMainValuesManager.getById(data.id)
                .then( (response) => {
    ...
    methods: {
      ...
      create: function(){
        ...
        this.objectMainValuesManager.create(createData)
                  .then( (response) => {
                    if (response.data !== false){
      ...
      update: function(){
        ...
        this.objectMainValuesManager.update(updateData)
                .then( (response) => {
                  if (response.data !== false){
                    if (response.data.error) {
                      this.showCustomLoaderDialog = true;
    ...
    computed: {
      ...
      short_nameErrors () {
        ...
        !this.$v.short_name.maxLength && errors.push(this.$store.state.t('Name must be at most 250 characters long'))
