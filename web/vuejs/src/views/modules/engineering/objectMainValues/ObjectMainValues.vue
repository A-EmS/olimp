<template>
  <div>
    <page-title :button-action-hide="getACL().create !== true" :createProcessName="createProcessName" :heading="$store.state.t('Object Main Values')" :subheading="$store.state.t('Object Main Values actions')" icon='pe-7s-global icon-gradient bg-happy-itmeo' :starShow=false></page-title>
    ...
    <b-card v-if="getACL().list === true" :title="$store.state.t('Object Main Values')" class="main-card mb-4">
      <b-row class="mb-3">
...
<script>
  ...
  import {ObjectMainValuesManager} from "@/managers/ObjectMainValuesManager";
  ...
  export default {
    ...
    data: () => ({
      ...
      updateItemListEventName: 'updateList:objectMainValues',
    ...
    created: function() {
      ...
      this.objectMainValuesManager = new ObjectMainValuesManager();
      this.getObjectMainValuesTypes();
    ...
    methods: {
      getObjectMainValuesTypes: function () {
        this.objectMainValuesManager.getAll()
                .then( (response) => {
      ...
      confirmDeleteRow: function(id, name){
        this.$eventHub.$emit(this.confirmatorInputProcessName, {
          ...
          confirmString: this.$store.state.t('Confirm delete') +  ' ' + this.$store.state.t('Object Main Value') +'..'+name,
          idToConfirm: id
        });
      },
      ...
      deleteRow: function(id){
        ...
        this.objectMainValuesManager.delete({id:id})
                .then( (response) => {
                  if(response.data !== false){
      ...
      setDefaultInterfaceData: function () {
        ...
        this.fields = [
          ...
          { key: 'name', label: this.$store.state.t('Object Main Value'), sortable: true},
